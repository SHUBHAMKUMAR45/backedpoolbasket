...
export const registerDeliveryNew = async (data) => {
  ...
  const partner = await DeliveryPartner.create({
    ...
    drivingLicense: data.drivingLicense || 'UNASSIGNED',
    isVerified: true
  });
...
export const forgotPassword = async (email) => {
  ...
  const otp = '1234'; // Simulated OTP for testing
  
  // Here we would normally email the OTP
...
export const resetPassword = async (email, otp, newPassword) => {
  ...
  if (otp !== '1234') throw new ApiError(400, 'Invalid OTP');
