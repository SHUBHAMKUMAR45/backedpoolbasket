...
export const toggleDeliveryPartnerStatus = async (partnerId) => {
  ...
  partner.isVerified = !partner.isVerified; // Suspend or activate
  await partner.save();
  return partner;
};
...
export const getDeliveryReports = async () => {
  ...
  const partners = await DeliveryPartner.find({}, 'earnings.codCollected');
  const totalPendingCod = partners.reduce((sum, p) => sum + (p.earnings?.codCollected || 0), 0);
...
export const assignPendingOrder = async (orderId, partnerId) => {
  ...
  const partner = await DeliveryPartner.findById(partnerId);
  if (!partner) throw new ApiError(404, 'Delivery partner not found');
  if (!partner.isVerified) throw new ApiError(400, 'Delivery partner is suspended or not verified');

  order.status = 'out_for_delivery';
  order.deliveryPartnerId = partner.user; // Link to user ID of partner
  order.deliveryStatus = 'picked_up'; // Adjust per schema if needed
  await order.save();
  return order;
};


export const assignRandomPendingOrder = async () => {
  const order = await Order.findOne({ status: { $in: ['pending', 'confirmed'] } });
  if (!order) throw new ApiError(404, 'No pending orders found');

  const partner = await DeliveryPartner.findOne({ isVerified: true });
  if (!partner) throw new ApiError(404, 'No active delivery partners found');

  order.status = 'out_for_delivery';
  order.deliveryPartnerId = partner.user;
