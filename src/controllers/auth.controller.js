...
export const registerDeliveryNew = asyncHandler(async (req, res) => {
  const result = await authService.registerDeliveryNew(req.body);
  res.status(201).json(new ApiResponse(201, result, 'Delivery partner registered successfully'));
});

export const forgotPassword = asyncHandler(async (req, res) => {
  const result = await authService.forgotPassword(req.body.email);
  res.status(200).json(new ApiResponse(200, result, 'Reset OTP sent'));
});
